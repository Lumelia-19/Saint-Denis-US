...
export const metadata: Metadata = {
  ...
  description:
    'Devenez partenaire du UFSD et soutenez un club formateur, populaire et ancré à Saint-Denis.',
  alternates: { canonical: '/partenaires' },
  openGraph: {
    title: 'Partenaires & Sponsoring | UFSD',
...
export default function PartenairesPage() {
  return (
    <section
      className="relative isolate overflow-hidden bg-surface pt-28 pb-16 lg:pt-24 lg:min-h-screen"
    ...
    >
      ...
      <div
        aria-hidden="true"
        className="absolute inset-0 -z-10 bg-[radial-gradient(circle_at_50%_42%,color-mix(in_srgb,var(--color-scrim)_96%,transparent)_0%,color-mix(in_srgb,var(--color-scrim)_88%,transparent)_34%,color-mix(in_srgb,var(--color-scrim)_56%,transparent)_58%,color-mix(in_srgb,var(--color-scrim)_18%,transparent)_100%)]"
      />
      <div
        aria-hidden="true"
        className="absolute inset-0 -z-10 bg-[linear-gradient(180deg,color-mix(in_srgb,var(--color-scrim)_92%,transparent)_0%,color-mix(in_srgb,var(--color-scrim)_30%,transparent)_52%,rgba(27,58,140,0.18)_82%,color-mix(in_srgb,var(--color-scrim)_96%,transparent)_100%)]"
      />
      ...
      <div className="relative z-10 mx-auto max-w-[1500px] px-5 sm:px-8 lg:min-h-[900px] lg:px-10 min-[1400px]:px-16">
        ...
        <Reveal delay={0.04} className="lg:absolute lg:left-[8%] lg:top-[36%] lg:w-[180px]">
          <div className="mb-8 max-w-xs lg:mb-0">
            <span className="mb-4 grid h-14 w-14 place-items-center rounded-full border border-flame/30 bg-panel/80 text-flame shadow-[0_18px_40px_-28px_rgba(13,27,75,0.55)] backdrop-blur">
              <Icon name="users" size={28} />
            </span>
            <p className="nums text-6xl font-black italic leading-none text-navy" style={{ fontFamily: 'var(--font-display)' }}>
        ...
        <Reveal delay={0.1} className="lg:absolute lg:right-[8%] lg:top-[36%] lg:w-[180px]">
          <div className="mb-8 max-w-xs lg:mb-0 lg:ml-auto">
            ...
            <p className="nums text-6xl font-black italic leading-none text-navy" style={{ fontFamily: 'var(--font-display)' }}>
              1
              <span className="ml-2 align-middle text-2xl not-italic text-navy">stade</span>
            </p>
            <p className="display-sm mt-1 text-2xl italic text-flame">Au cœur de Saint-Denis</p>
            <p className="mt-4 text-sm leading-relaxed text-navy/78">Le Stade de France, notre fierté, notre force.</p>
          </div>
        </Reveal>
        ...
        <div className="relative mx-auto flex w-full max-w-[640px] flex-col items-center justify-center py-8 text-center lg:h-[640px] lg:w-[640px] lg:py-0">
          ...
          <div
            aria-hidden="true"
            className="pointer-events-none absolute left-1/2 top-1/2 hidden h-[360px] w-[360px] -translate-x-1/2 -translate-y-1/2 rounded-full bg-scrim/40 blur-2xl lg:block"
          />

          {NODES.map((node) => (
            <div
              key={node.title}
              className="mb-3 flex items-center gap-3 rounded-[1rem] border border-royal/20 bg-panel/80 px-4 py-3 text-left shadow-[0_22px_44px_-30px_rgba(13,27,75,0.72)] backdrop-blur lg:absolute lg:z-10 lg:mb-0 lg:flex-col lg:justify-center lg:h-32 lg:w-32 lg:gap-1 lg:rounded-full lg:border-2 lg:border-royal lg:px-3 lg:py-3 lg:text-center lg:ring-4 lg:ring-white/70"
              style={nodePos(node.angle)}
            >
              <span className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-panel text-navy lg:h-auto lg:w-auto lg:shrink lg:bg-transparent">
                <Icon name={node.icon} size={24} strokeWidth={1.8} />
              </span>
              <p className="display-sm whitespace-pre-line text-base italic leading-[0.98] text-navy lg:text-[0.78rem]">
                {node.title}
              </p>
            </div>
          ))}
          ...
          <div className="relative z-20 mx-auto max-w-md pt-2 lg:pt-0">
            <h1 id="partenaires-title" className="hero-title text-navy lg:text-[3.6rem]">
              Partenaires
              <br />
              <span className="text-flame">&amp; Sponsoring</span>
            </h1>
            <p className="mx-auto mt-5 max-w-sm text-[0.92rem] leading-relaxed text-navy/78">
        ...
        <div className="mt-10 grid gap-4 lg:absolute lg:bottom-10 lg:left-1/2 lg:mt-0 lg:w-[780px] lg:-translate-x-1/2 lg:grid-cols-3">
          {BENEFITS.map((item, i) => (
            <Reveal key={item.title} delay={0.32 + i * 0.08}>
              <Link
                href="/contact"
                className="group relative block h-full min-h-[170px] overflow-hidden rounded-2xl border border-royal/14 bg-panel/80 p-6 shadow-[0_22px_54px_-34px_rgba(13,27,75,0.65)] backdrop-blur transition duration-300 hover:-translate-y-1"
              >
                ...
                <h3 className="display-sm text-2xl italic text-navy">{item.title}</h3>
